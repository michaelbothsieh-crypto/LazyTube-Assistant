...
def process_with_notebooklm(video_url, title):
    ...
    try:
        ...
        if res.returncode == 0:
            # 3. 執行查詢 (修正語法：加入 ask 子指令)
            query = "請用繁體中文列出這部影片的 3 到 5 個核心重點，並加上影片標題"
            # 嘗試兩種可能語法
            print("🔄 執行摘要查詢...")
            res = run_nlm("query", "ask", notebook_id, query)
            if res.returncode != 0:
                res = run_nlm("query", notebook_id, query) # 備案語法
...
def main():
    print("="*50)
    print(f"🚀 LazyTube-Assistant [VERSION: 2026.03.06.39 - SYNTAX FIX]")
    print("="*50)

    # 1. 認證佈署 (維持 VERSION 38 成功的邏輯)
    ...
    # --- [ 測試模式：使用確定可抓取的影片 ] ---
    print("🧪 啟動修正語法後的測試摘要...")
    test_url = "https://www.youtube.com/watch?v=t5RVVWUS9nk" # 您清單中最近的一部
