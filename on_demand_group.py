...
def main():
    ...
    # 除錯資訊：印出所有存在的群組 ID 與其類型
    print(f"📊 載入訂閱資料成功")
    print(f"   目標 Chat ID: {chat_id} (type: {type(chat_id)})")
    print(f"   資料庫中群組: {[{'id': k, 'type': str(type(k))} for k in all_subs.keys()]}")
    
    # 強制轉型匹配
    group_subs = all_subs.get(str(chat_id)) or all_subs.get(int(chat_id) if str(chat_id).lstrip('-').isdigit() else None)
